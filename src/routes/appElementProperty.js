...
const { create, update, get, validate } = require("../models-mssql/AppElementProperty");

router.get("/", async (req, res, next) => {
  console.log("/appElementProperty GET call", req.query.appElementId);
  try {
    if (req.query.appElementId) {
      const data = await get(req.query.appElementId);
      next(new QueryResult(data));
    } else {
      next({ error: "appElementId can not be null!", status: HttpStatus.BAD_REQUEST });
    }
  } catch (err) {
...
router.get("/:id", async (req, res, next) => {
  console.log("/api/appElementProperty GET/{id} call");
  try {
...
router.post("/", async (req, res, next) => {
  console.log("/api/appElementProperty POST call");
...
router.put("/:id", async (req, res, next) => {
  console.log("/api/appElementProperty PUT call", req.params.id);
