...
const { get, getSubjects, validate, create, update, del } = require("../models-mssql/ClassSectionSyllabusClassCombinations");

router.get("/", async (req, res, next) => {
  console.log("/classSectionSyllabusClassCombinations GET call");
  try {
...
router.get("/subjects", async (req, res, next) => {
  console.log("/classSectionSyllabusClassCombinations GET/subjects call");
  try {
    const data = await getSubjects(req.query, req.user);
    next(new QueryResult(data));
  } catch (err) {
    // err.status = HttpStatus.BAD_REQUEST;
    next({ error: err, status: HttpStatus.BAD_REQUEST });
...
router.post("/", async (req, res, next) => {
  console.log("/classSectionSyllabusClassCombinations POST call");
...
router.delete("/:id", async (req, res, next) => {
  console.log("/classSectionSyllabusClassCombinations DELETE call", req.params.id);
  const data = await del(req.params.id);
  next(new QueryResult(data));
}, returnStateHandler);

router.put("/:id", async (req, res, next) => {
  console.log("/classSectionSyllabusClassCombinations PUT call", req.params.id);
