...
const { get, validate, create, update, del } = require("../models-mssql/Classes");

router.get("/", async (req, res, next) => {
  console.log("/Classes GET call");
  try {
...
router.delete("/:id", async (req, res, next) => {
  console.log("/Classes DELETE call", req.params.id);
  const data = await del(req.params.id);
  next(new QueryResult(data));
}, returnStateHandler);

router.put("/:id", async (req, res, next) => {
  console.log("/Classes PUT call", req.params.id);
