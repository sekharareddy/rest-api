...
const { get, validate, create, update, del } = require("../models-mssql/Exam");

router.get("/", async (req, res, next) => {
  console.log("/exams GET call");
  try {
...
router.delete("/:id", async (req, res, next) => {
  console.log("/exams DELETE call", req.params.id);
  const data = await del(req.params.id);
  next(new QueryResult(data));
}, returnStateHandler);

router.put("/:id", async (req, res, next) => {
  console.log("/exams PUT call", req.params.id);
