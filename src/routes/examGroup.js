...
const { get, validate, create, update, del } = require("../models-mssql/ExamGroup");

router.get("/", async (req, res, next) => {
  console.log("/examGroups GET call");
  try {
...
router.post("/", async (req, res, next) => {
  console.log("/examGroups POST call");
...
router.delete("/:id", async (req, res, next) => {
  console.log("/examGroups DELETE call", req.params.id);
  const data = await del(req.params.id);
  next(new QueryResult(data));
}, returnStateHandler);

router.put("/:id", async (req, res, next) => {
  console.log("/examGroups PUT call", req.params.id);
