...
const { get, validate, create, update, del } = require("../models-mssql/ExpenseAccount");

console.log(router);
router.get("/", async (req, res, next) => {
  console.log("/ExpenseAccount GET call");
  try {
...
router.get("/:id", async (req, res, next) => {
  console.log(30, "/ExpenseAccount GET call");
  try {
...
router.post("/", async (req, res, next) => {
  console.log(42, "/ExpenseAccount POST call");
...
router.delete("/:id", async (req, res, next) => {
  console.log("/ExpenseAccount DELETE call", req.params.id);
  const data = await del(req.params.id);
  next(new QueryResult(data));
}, returnStateHandler);

router.put("/:id", async (req, res, next) => {
  console.log("/ExpenseAccount PUT call", req.params.id);
