...
const { get, validate, create, update, del } = require("../models-mssql/Expenses");

router.get("/", async (req, res, next) => {
  console.log("/Expenses GET call");
  try {
...
router.get("/:id", async (req, res, next) => {
  console.log(30, "/Expenses GET call");
  try {
...
router.post("/", async (req, res, next) => {
  console.log(42, "/Expenses POST call");
...
router.delete("/:id", async (req, res, next) => {
  console.log("/Expenses DELETE call", req.params.id);
  const data = await del(req.params.id);
  next(new QueryResult(data));
}, returnStateHandler);

router.put("/:id", async (req, res, next) => {
  console.log("/Expenses PUT call", req.params.id);
