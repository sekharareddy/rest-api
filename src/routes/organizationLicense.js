...
const { create, del, getById, get, update, validate } = require("../models-mssql/OrganizationLicense");

router.get("/", async (req, res, next) => {
  console.log("/OrgLicense GET call");
  try {
    if (req.query.orgId) {
      const data = await get(req.query.orgId);
      next(new QueryResult(data));
    } else {
      next({ error: "orgId can not be null!", status: HttpStatus.BAD_REQUEST });
    }
  } catch (err) {
...
router.post("/", async (req, res, next) => {
  console.log("/OrgLicense POST call");
  // prepare body
  // let body;
...
router.put("/:id", async (req, res, next) => {
  console.log("/OrgLicense PUT call", req.params.id);
  // prepare body
...
router.delete(
  "/:id",
  async (req, res, next) => {
    console.log("/OrgLicense DELETE call", req.params.id);
