...
const { create, del, update, get, getById, validate } = require("../models-mssql/PageRole");

router.get("/", async (req, res, next) => {
  console.log("/pageRole GET call", req.query.tenantId);
  try {
...
router.put("/:id", async (req, res, next) => {
  console.log("/api/pageRole PUT call", req.params.id);
  let roleCheck = false;
  if (req.user && req.user.userRoles) {
    req.user.userRoles.forEach((userRole) => {
      if (userRole.roleName == "SysAdmin") {
...
router.delete(
  "/:id",
  async (req, res, next) => {
    console.log("/api/pageRole DELETE call", req.params.id);
    let roleCheck = false;
    if (req.user && req.user.userRoles) {
      req.user.userRoles.forEach((userRole) => {
        if (userRole.roleName == "SysAdmin") {
