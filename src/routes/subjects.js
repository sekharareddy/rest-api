...
const { get, validate, create, update, del } = require("../models-mssql/Subjects");

router.get("/", async (req, res, next) => {
  console.log("/subjects GET call");
  try {
    const data = await get(req.query.subjectId, req.query, req.user);
...
router.post("/", async (req, res, next) => {
  console.log("/subjects POST call");
...
router.delete("/:id", async (req, res, next) => {
  console.log("/subjects DELETE call", req.params.id);
  const data = await del(req.params.id);
  next(new QueryResult(data));
}, returnStateHandler);

router.put("/:id", async (req, res, next) => {
  console.log("/subjects PUT call", req.params.id);
