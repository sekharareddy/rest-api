...
const { get, validate, create, update, del } = require("../models-mssql/SyllabusCombination");

router.get("/", async (req, res, next) => {
  console.log("/syllabusCombination GET call");
  try {
    const data = await get(req.query.syllabusCombinationId, req.query, req.user);
...
router.post("/", async (req, res, next) => {
  console.log("/syllabusCombination POST call");
...
router.delete("/:id", async (req, res, next) => {
  console.log("/syllabusCombination DELETE call", req.params.id);
  const data = await del(req.params.id);
  next(new QueryResult(data));
}, returnStateHandler);

router.put("/:id", async (req, res, next) => {
  console.log("/syllabusCombination PUT call", req.params.id);
