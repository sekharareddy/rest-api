...
const { get, validate, create, update, del } = require("../models-mssql/SyllabusCombinationSubject");

router.get("/", async (req, res, next) => {
  console.log("/syllabusCombinationSubject GET call");
  try {
...
router.post("/", async (req, res, next) => {
  console.log("/syllabusCombinationSubject POST call");
...
router.delete("/:id", async (req, res, next) => {
  console.log("/syllabusCombinationSubject DELETE call", req.params.id);
  const data = await del(req.params.id);
  next(new QueryResult(data));
}, returnStateHandler);

router.put("/:id", async (req, res, next) => {
  console.log("/syllabusCombinationSubject PUT call", req.params.id);
...
router.patch("/:id", async (req, res, next) => {
  console.log("/syllabusCombinationSubject PATCH call", req.params.id);
  await validate(req.body, req.params.id).then(async (dataIn) => {
    dataIn = await update(dataIn);
    next(new QueryResult(dataIn));
  }).catch((err) => {
