...
const { get, validate, create, update, del } = require("../models-mssql/SyllabusCombinationSubjectExam");

router.get("/", async (req, res, next) => {
  console.log("/syllabusCombinationSubjectExam GET call");
  try {
...
router.post("/", async (req, res, next) => {
  console.log("/syllabusCombinationSubjectExam POST call");
...
router.delete("/:id", async (req, res, next) => {
  console.log("/syllabusCombinationSubjectExam DELETE call", req.params.id);
  const data = await del(req.params.id);
  next(new QueryResult(data));
}, returnStateHandler);

router.put("/:id", async (req, res, next) => {
  console.log("/syllabusCombinationSubjectExam PUT call", req.params.id);
...
router.patch("/:id", async (req, res, next) => {
  console.log("/syllabusCombinationSubjectExam PATCH call", req.params.id);
  await validate(req.body, req.params.id).then(async (dataIn) => {
    dataIn = await update(dataIn);
    next(new QueryResult(dataIn));
  }).catch((err) => {
